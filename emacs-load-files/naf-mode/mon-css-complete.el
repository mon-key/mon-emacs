...
;;; :CHANGED :FUNCTION `css-pos-for-x-popup-menu'
;;;          -- Now directly let* binds the relevant `posn-*' return values of
;;;            `posn-at-point' instead of dbinding.
;;;
...
;;;
;;; FILE-CREATED:
;;; <Timestamp: #{2010-03-31T17:56:42-04:00Z}#{10133} - by MON>
;;;
;;; ==============================
...
W-REGION-FNCN is a function returning a list the car and cadr of which satisfy
the predicate `integer-or-marker-p' and which has the form:\n
 ( <REGION-END> <REGION-BEG> {...} )\n
W-APPLIED-FNCN is a function to apply to the returned list.
Its first two args should be buffer positions, e.g. beginning and end.\n
:EXAMPLE\n\n\(pp-macroexpand-expression
...
`css-maybe-part-tag-at-point', `css-maybe-part-at-at-point'.\n▶▶▶"
  ...
  ;;      (unless (or (null ,css-rgn)
  ;;                  (and (consp ,css-rgn)
  ;;                       (integer-or-marker-p (car ,css-rgn))
  ;;                       (integer-or-marker-p (cadr ,css-rgn))
  ;;                       (= (car ,css-rgn) (cadr ,css-rgn))))
  ;;        (apply ',w-applied-fncn ,css-rgn)))))
  ;;
  ;;
  ;; :NOTE If W-REGION-FNCN's return value has a `cddr' its an &rest.
  ;; We don't do anything with this but could branch on an &rest with:
  ;; (if (cddr ,css-rgn) {...} just before: (apply ',w-applied-fncn {...}
  (let ((css-rgn (make-symbol "css-rgn")))
    `(let ((,css-rgn (funcall ',w-region-fncn)))
       ;; Bail if we don't get a list:
       (if (not (null ,css-rgn))
           ;; Test we got the right kind of list before applying W-REGION-FNCN
           (if (and 
                ;; We got a list
                (consp ,css-rgn)
                ;; First two elts are buffer positions (putative).
                (integer-or-marker-p (car  ,css-rgn))
                (integer-or-marker-p (cadr ,css-rgn))
                ;; We don't signal an args-out-of-range error e.g.:
                ;; (buffer-substring-no-properties 0 0)
                (not (= (car ,css-rgn) (cadr ,css-rgn))))
               ;; Maybe it matters to a caller that the region positions
               ;; are passed in ascending order. So we make sure they are.
               (apply ',w-applied-fncn (if (> (car ,css-rgn) (cadr ,css-rgn))
                                           `(,(cadr ,css-rgn) ,(car ,css-rgn))
                                         `(,(car ,css-rgn) ,(cadr ,css-rgn)))))
         nil))))
...
;;; ==============================
;;; :CHANGESET 2053
;;; :CREATED <Timestamp: #{2010-08-07T11:03:11-04:00Z}#{10316} - by MON KEY>
...
`mon-help-css-mode', `mon-help-css-check', `mon-help-css-color'.\n▶▶▶"
 ...
 (let* ((cpfxpm (posn-at-point))
        (cpfxpm-x-y (posn-x-y cpfxpm))
        (cpfxpm-win (posn-window cpfxpm)))
   `((,(+ *css-complete-popup-pos-x-offset* (car cpfxpm-x-y)) ,(cdr cpfxpm-x-y))
     ,cpfxpm-win)))
...
This is predicate for `css-complete' to trigger completion with
`css-complete-value'.\n
...
This is predicate for `css-complete' to trigger `css-complete-pseudo'
completion.\n
...
This is a predicate for `css-complete' to trigger completion with
`css-complete-at'.\n
:SEE-ALSO `css-at-p-generate', `mon-help-css-complete',
`mon-help-css-properties', `mon-help-css-mode', `mon-help-css-check',
`mon-help-css-color'.\n▶▶▶"
...
(defun css-after-at-p ()
  "Retrun non-nil if `looking-back' at a CSS @VALUE.\n
A `css-complete' predicate to trigger `css-complete-after-at' completion.\n
:EXAMPLE\n\n\(save-excursion \(end-of-line 2\) \(css-after-at-p\)\)
@charset \n
This is a predicate for `css-complete' to trigger `css-complete-after-at'
completion.\n
:SEE-ALSO `css-at-p-generate', `mon-help-css-complete',
`mon-help-css-properties', `mon-help-css-mode', `mon-help-css-check',
`mon-help-css-color'.\n▶▶▶"
...
(defun css-complete-after-at ()
  "If we're looking-back on an attribute perform an appropreate action.\n
...
(defun css-complete-media ()
  "Find `*css-complete-media-ids*' completions with `css-media-popup-completions'
